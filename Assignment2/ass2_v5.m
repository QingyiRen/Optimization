...
options = optimoptions('fmincon','Algorithm','sqp','MaxFunctionEvaluations',8e4,'TolFun',5e-4, 'ConstraintTolerance', 1e-4);
[x1,fval1,exitflag1,output1] = fmincon(@(x1)myfun(x1,C_u_d,C_o1_d,C_d_u,C_d_o3,C_d_o2,C_d_o1,alpha_enter_o1_d,alpha_enter_u_d),ub,[],[],[],[],lb,ub,[],options)
...
for k = 1:60
        ...
        alpha_leave_u_d_o1(k) = min([beta_u_d_o1*mu_u_d*x(k)/c, q_u_d_o1(k)/c + beta_u_d_o1*alpha_arrive_u_d(k), C_d_o1(k)/c]);
        alpha_leave_u_d_o2(k) = min([beta_u_d_o2*mu_u_d*x(k)/c, q_u_d_o2(k)/c + beta_u_d_o2*alpha_arrive_u_d(k), C_d_o2(k)/c]);
        alpha_leave_u_d_o3(k) = min([beta_u_d_o3*mu_u_d, q_u_d_o3(k)/c + beta_u_d_o3*alpha_arrive_u_d(k), C_d_o3(k)/c]);
    
        alpha_leave_o1_d_u(k) = min([beta_o1_d_u*mu_o1_d, q_o1_d_u(k)/c + beta_o1_d_u*alpha_arrive_o1_d(k), C_d_u(k)/c]);
        alpha_leave_o1_d_o2(k) = min([beta_o1_d_o2*mu_o1_d*(c-x(k))/c, q_o1_d_o2(k)/c + beta_o1_d_o2*alpha_arrive_o1_d(k), C_d_o2(k)/c]);
        alpha_leave_o1_d_o3(k) = min([beta_o1_d_o3*mu_o1_d*(c-x(k))/c, q_o1_d_o3(k)/c + beta_o1_d_o3*alpha_arrive_o1_d(k), C_d_o3(k)/c]);

        n_u_d(k+1)=n_u_d(k)+(alpha_enter_u_d(k)-(alpha_leave_u_d_o1(k)+alpha_leave_u_d_o2(k)+alpha_leave_u_d_o3(k)))*c;
        n_o1_d(k+1)=n_o1_d(k)+(alpha_enter_o1_d(k)-(alpha_leave_o1_d_u(k)+alpha_leave_o1_d_o2(k)+alpha_leave_o1_d_o3(k)))*c;

        q_u_d_o1(k+1) = q_u_d_o1(k)+(beta_u_d_o1*alpha_arrive_u_d(k)-alpha_leave_u_d_o1(k))*c;
        q_u_d_o2(k+1) = q_u_d_o2(k)+(beta_u_d_o2*alpha_arrive_u_d(k)-alpha_leave_u_d_o2(k))*c;
        q_u_d_o3(k+1) = q_u_d_o3(k)+(beta_u_d_o3*alpha_arrive_u_d(k)-alpha_leave_u_d_o3(k))*c;

        q_o1_d_u(k+1) = q_o1_d_u(k)+(beta_o1_d_u*alpha_arrive_o1_d(k)-alpha_leave_o1_d_u(k))*c;
        q_o1_d_o2(k+1) = q_o1_d_o2(k)+(beta_o1_d_o2*alpha_arrive_o1_d(k)-alpha_leave_o1_d_o2(k))*c;
        q_o1_d_o3(k+1) = q_o1_d_o3(k)+(beta_o1_d_o3*alpha_arrive_o1_d(k)-alpha_leave_o1_d_o3(k))*c;  
...
function f = myfun(x,C_u_d,C_o1_d,C_d_u,C_d_o3,C_d_o2,C_d_o1,alpha_enter_o1_d,alpha_enter_u_d)
    ...
    for k = 1:60
        ...
        %func(k+1) = -c^2*(alpha_leave_u_d_o1(k)+alpha_leave_u_d_o2(k)+alpha_leave_u_d_o3(k)+alpha_leave_o1_d_u(k)+alpha_leave_o1_d_o2(k)+alpha_leave_o1_d_o3(k));
